# -*- coding: utf-8 -*-
from ui.ZHUpCK import Ui_LoginDialog
...
class LoginDialog(QDialog):
    ...
    def ob(self):
        Vals = []
        Vats = ['Bank_Name', 'Balance', 'Opening_Date', 'Interest_Rate', 'Currency_Type']
        ...
        if self.ui.nOpening_Date.isChecked():
            epy[2] = 1
        if self.ui.nInterest_Rate.isChecked():
            epy[3] = 1
        if self.ui.nCurrency_Type.isChecked():
            epy[4] = 1

        res1 = self.db.execute("select count(*) from Checking where Account_Type = '1' and Bank_Name = '" + Vals[0] + "' and Client_ID in (select Client_ID from Own where Account_ID = '" + self.IDf + "');")
        res2 = self.db.execute("select count(*) from Bank where Bank_Name = '" + Vals[0] + "';")
        if res1[0][0] > 0 and epy[0] == 1:
            critical(self, "此账户的持有者之一已在该银行持有存款账户")
            ...
            for i in range(3, 5):
                if epy[i] == 1:
                    if Vals[i] is not '':
                        self.db.execute("update Saving_Account set " + Vats[i] + " = '" + Vals[i] + "' where Account_ID = '" + self.IDf + "';")
                    else:
                        self.db.execute("update Saving_Account set " + Vats[i] + " = null where Account_ID = '" + self.IDf + "';")
            if epy[0] == 1:
